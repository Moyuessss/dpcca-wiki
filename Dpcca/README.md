...
- 状态：**测试版 2.1**（2026-09-12 新增「公告弹窗」功能）
- 首版本里程碑：v1.0.0（2026-09-07）
...
- 本地快照：`releases/dpcca-v1.0.0-test-20260907/`（含源码包与清理归档）

> 当前各公开页右上角均已显示「测试版 v2.1」徽标。测试期间数据/功能可能调整，反馈请走 README 渠道。
...
- 登录方式：
  ...
  - **管理后台**：账号密码（Dpcca-admin / Dpcca2026）由云函数 `admin_login` 校验，写库必须经云函数
...
- **管理后台写库**：`anime_library` 客户端已禁写，新增/修改/删除必须调用云函数 `admin_login`（内部校验 Dpcca-admin / Dpcca2026 后直连数据库），前端无法绕过
...
├── announcement.js   # v2.1 公告弹窗组件（三页共用，含管理后台预览）
└── README.md         # 本文档
```
