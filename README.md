...
- 单集评分柱状图：详情页 SVG 图表直观展示每集得分趋势（含均分参考线，悬停查看短评）；「查看更多评分」可查看全站用户对本番各集的评分汇总
- 系列季度关联：同系列多季度番剧互相跳转（数据由后台配置系列名 + 季数）
...
- 同款筛选、详情弹窗、PV 按钮、单集评分图表、系列季度关联、评论墙
...
- 评分预设：后台预设的筛选模板一键应用（年份 / 月份 / 季度 / 来源 / 类型 / 分制 / 模式）
- 评分顺序排序：拖拽卡片自定义评分顺序，支持按开播日期 / 按标题快捷排序
...
- 番剧增删改：标题、季度、类型、标签、简介、制作信息、封面、PV 链接、隐藏关键词、系列名与季数等
- 放送日程管理：各集放送日期手动微调，「本周停播」一键将未播集顺延一周（跨年自动带年份）
- 评分预设管理：为评分室配置一键筛选模板
...
| `site_config` | 站点级配置（公告弹窗单文档 `announcement`；评分预设单文档 `rating_presets`） | 读：所有人；写：`false`（仅云函数可写） |
...
| 云函数 | 用途 |
| --- | --- |
| `admin_login` | 管理后台登录校验 + 番剧库写操作 + 公告配置 + 评分预设 |
...
- **v4.0**（2026-09-22 ~ 09-23）：评分预设——管理后台配置「筛选 + 番剧勾选 + 评分顺序」模板，评分室一键应用；单集评分柱状图增强——柱顶标注分数、「查看更多评分」全站单集评分汇总柱状图、保存 / 删除评分后评分表保持展开不收起
- **v3.1**（2026-09-15）：番剧 PV 链接按钮（前台 / 评分室 / 管理后台三端联动）；页脚新增 CC BY-NC-SA 4.0 数据来源署名
- **v3.0**（2026-09-14）：季度筛选与全维度多选筛选；房主密码锁与房间号隐藏；统一短评评论框组件
- **v2.1**（2026-09-12）：全站公告弹窗（内容指纹静默机制）
- **v1.0.0**（2026-09-07）：首个可用版本
